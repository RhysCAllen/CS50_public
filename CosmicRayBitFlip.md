...
Bit flips are similar to DNA mutations called SNPs, or single nucleotide polymorphsms. In this type of mutation, a single letter of DNA is changed to a different letter, such as A -> T. SNPs are common in cells through natural mutagenic processes like DNA replicaton. Just like bit flips, SNPs have the potential to do harm, by changing the sequence of data used to store information. Changing a 0 to 1 could stop a computer program from working. Changing a DNA nucleotide from A to T could stop a gene from working.
